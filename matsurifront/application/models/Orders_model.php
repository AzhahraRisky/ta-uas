...
class Orders_model extends CI_Model
{
    ...
    public function process()
    {
        ...
        //kurangi stok menu
        foreach ($this->cart->contents() as $item) {
            $hasil = $this->db->where('kd_barang', $item['id'])->limit(1)->get('menu')->result_array();
            ...
            $this->db->update('menu', $data, array('kd_barang' => $item['id']));
        }
        return $order_id;
    }
...

    {
        ...
        $this->db->select('menu.nm_barang, menu.harga, detail_penjualan.jml_brg');

        $this->db->from('detail_penjualan');

        $this->db->join('menu', 'detail_penjualan.kd_brg=menu.kd_barang');
